 ...
 */
public class TCLivePlayerActivity extends TCBaseActivity implements ITXLivePlayListener,
        View.OnClickListener, TCPlayerMgr.PlayerListener, TCInputTextMsgDialog.OnTextSendListener,
        TCChatRoomMgr.TCChatRoomListener {
    ...
    private void initVodView() {
        ...
        mtvPuserName.setText(TCUtils.getLimitString(mPusherId, 10));
...
//        }
    ...
    private void startPlay() {
        ...
        if (mIsLivePlay) {
            ...
            result = mTXLivePlayer.startPlay(mPlayUrl, TXLivePlayer.PLAY_TYPE_VOD_FLV);
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_back:
                ...
                rstData.putExtra(TCConstants.PUSHER_ID, mUserId);
                setResult(0,rstData);
                finish();
                break;
            case R.id.btn_like:
                ...
                if (mLikeFrequeControl.canTrigger()) {
                    mHeartCount++;
                    //向后台发送点赞信息
                    mTCPlayerMgr.addHeartCount(mUserId);
                    //向ChatRoom发送点赞消息
